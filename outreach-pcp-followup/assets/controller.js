  ...
  fetch('/deployment/check-aws-application')
    ...
    .then((status) => {
      ...
      if (status === 'NOT-DEPLOYED') {
        ...
        $('#aws-application-open').attr('href', `https://console.aws.amazon.com/cloudformation/`);
    ...
    .catch((err) => {
      console.log('An error occurred when attempting to check the AWS Resources', err);
    });
}
  ...
  fetch('/deployment/check-aws-bucket')
    ...
    .then((status) => {
      ...
      if (status === 'NOT-DEPLOYED') {
        ...
        $('#aws-bucket-open').attr('href', `https://console.aws.amazon.com/s3/buckets/${status}`);
...
function deployStudioFlow(e) {
  ...
  fetch('/deployment/deploy-studio-flow')
    ...
    .catch((err) => {
      console.log('An error occurred creating Studio Flow', err);
      $('#flow-deploy .button').removeClass('loading');
...
// --------------------------------------------------------------------------------
function renderIntegrationInformation() {
