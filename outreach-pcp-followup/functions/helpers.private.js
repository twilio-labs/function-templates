...
const APPLICATION_NAME = 'outreach-pcp-followup';
const CONSTANTS = {
  'APPLICATION_NAME': APPLICATION_NAME,
  'APPLICATION_FILENAME_PATTERN_OUTREACH': 'patient{patient_id}-outreach.json',
  'APPLICATION_FILENAME_PATTERN_QUEUE': 'outreach-{outreach_id}.csv',
  'APPLICATION_FILENAME_PATTERN_RESPONSE': 'patient{patient_id}-response-{epoch}.json',
  ...
  'AWS_LAMBDA_SEND_OUTREACH_BASE_NAME': `twilio-send-${APPLICATION_NAME}`,
  'AWS_GLUE_CRAWLER_BASE_NAME': `twilio-${APPLICATION_NAME}`,
  'AWS_GLUE_DATABASE': 'outreach_pcp_followup',
};
...
async function retrieveParameter(context, key) {
  ...
  switch (key)
  {
    case 'AWS_ACCESS_KEY_ID':
    {
      // ---------- get aws clients
      const options = {
        accessKeyId: await retrieveParameter(context, 'AWS_DEPLOYER_AWS_ACCESS_KEY_ID'),
        secretAccessKey: await retrieveParameter(context, 'AWS_DEPLOYER_AWS_SECRET_ACCESS_KEY'),
    ...
    case 'AWS_CF_STACK_APPLICATION':
    {
      ...
      const name = CONSTANTS.AWS_CF_APPLICATION_STACK_BASE_NAME + '-' + APPLICATION_CUSTOMER_CODE;
      return name;
    }
    case 'AWS_CF_STACK_BUCKET':
    {
      const APPLICATION_CUSTOMER_CODE = await retrieveParameter(context, 'APPLICATION_CUSTOMER_CODE');
      const name = CONSTANTS.AWS_CF_BUCKET_STACK_BASE_NAME + '-' + APPLICATION_CUSTOMER_CODE;
      return name;
    }
    case 'AWS_GLUE_CRAWLER':
    {
      const APPLICATION_CUSTOMER_CODE = await retrieveParameter(context, 'APPLICATION_CUSTOMER_CODE');
      const name = CONSTANTS.AWS_GLUE_CRAWLER_BASE_NAME + '-' + APPLICATION_CUSTOMER_CODE;
      return name;
    }
    case 'AWS_LAMBDA_SEND_OUTREACH':
    {
      const APPLICATION_CUSTOMER_CODE = await retrieveParameter(context, 'APPLICATION_CUSTOMER_CODE');
      const name = CONSTANTS.AWS_LAMBDA_SEND_OUTREACH_BASE_NAME + '-' + APPLICATION_CUSTOMER_CODE;
      return name;
    }
    case 'AWS_S3_BUCKET':
    {
      const APPLICATION_CUSTOMER_CODE = await retrieveParameter(context, 'APPLICATION_CUSTOMER_CODE');
      const name = CONSTANTS.AWS_S3_BUCKET_BASE_NAME + '-' + APPLICATION_CUSTOMER_CODE;
      return name;
    }
    ...
    case 'TWILIO_FLOW_SID':
    {
      ...
      if (flow_sid != null) {
        ...
        console.log('STUDIO FLOW NOT DEPLOYED...');
        return null;
      }
    }
    case 'TWILIO_SERVICE_SID':
    {
      ...
      if (service_sid != null) {
        ...
        console.log('        console.log(\'SERVICE NOT DEPLOYED...\');\n');
        return null;
      }
    }
    default:
