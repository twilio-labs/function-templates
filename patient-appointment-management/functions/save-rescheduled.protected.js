...
// . REPLACE in STATE   (disposition=QUEUED)
...
exports.handler = async function(context, event, callback) {
    ...
    for (let d of ['QUEUED', 'REMINDED-1', 'REMINDED-2', 'EXPIRED']) {
        try {
            const params = {
    ...
    const new_state_file_s3key = state_file_s3key.replace('{DISPOSITION}', 'QUEUED');
