...
async function verifyOTP() {
    ...
    let otpInputs = document.querySelectorAll('input.otp-digit');
    let otpValues = [];
    otpInputs.forEach(input => {
        otpValues.push(input.value);
    });

    let code = otpValues.join('')
    ...
    if (result.success) {
        console.log('Verification successful. Fetching user data every 15 seconds...');
    ...
    try {
        ...
        if (result.prefillData.error_code === null) {
            ...
            if (maxCalls < 50) {
                ...
                alert('Unable to call Lookup API successfully. Do you have access to pre-fill?')
            }
        }
    } catch (err) {
        console.error(err);
