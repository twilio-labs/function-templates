const funlet = require('../functions/funlet-simple-menu.protected');
...
test(
  '[SIMPLE-MENU-INPUT-ERROR-MESSAGE-3] ' +
...
test(
  '[SIMPLE-MENU-INPUT-ERROR-MESSAGE-4] ' +
...
test('[SIMPLE-MENU-INPUT-VOICE-2] Read Voice from Environment', () => {
  expect(funlet.input.getVoice({}, { FUNLET_MENU_VOICE: WOMAN }, {})).toEqual(
    WOMAN
  );
});
...
test('[SIMPLE-MENU-INPUT-OPTIONS-1] Read Single Option from Event', () => {
  expect(funlet.input.getOptions({ Options: ACTION0 }, {}, {})).toEqual({
    0: ACTION0,
  });
});

test(
  '[SIMPLE-MENU-INPUT-OPTIONS-2] ' +
...
test(
  '[SIMPLE-MENU-OUTPUT-GATHER-DIGITS-1] ' +
...
<Redirect>${ACTION12345}</Redirect>\
  ...
  funlet.handler(
    {},
    { Digits: '12345', 'Options[12345]': ACTION12345 },
    callback
  );
});
