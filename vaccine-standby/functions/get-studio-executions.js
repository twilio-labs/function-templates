  ...
  client.studio.v1
    ...
    .then((executions) => {
      if (executions.length > 0) {
        ...
        const promises = endedExecutions.map(
          (e) =>
              ...
              client.studio
                ...
                .then((executionContext) => {
                  try {
                    ...
                    resident.essential_worker =
                      ec.widgets.EssentialWorker.inbound.Body;
                    resident.work_from_home =
                      ec.widgets.WorkFromHome.inbound.Body;
                    resident.long_term_care =
                      ec.widgets.LongTermCare.inbound.Body;
                    resident.congregate_setting =
                      ec.widgets.CongregateSetting.inbound.Body;
                    resident.health_condition =
                      ec.widgets.HealthCondition.inbound.Body;
                    resident.notification_preference =
                      ec.widgets.NotificationPreference.inbound.Body;
                    resident.language_preference =
                      ec.widgets.LanguagePreference.inbound.Body;
                    resolve(resident);
                  } catch (err) {
                    console.log(err);
