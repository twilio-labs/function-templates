...
messageBox.addEventListener('input', (evt) => {
  if (messageBox.value.length > 0) {
    let segmentCount = 1;
    const [maxCharInMessage, maxCharInSegment] = /[^\u0000-\u00ff]/.test(
      messageBox.value
    )
      ? [70, 67]
      : [160, 153];
    ...
    segmentCountMessage.innerText +=
      maxCharInMessage === 70 ? ' and Unicode Detected' : '';
  } else {
    segmentCountMessage.innerText = '';
  }
});
...
viewFilter.forEach((selection) => {
  selection.addEventListener('click', (evt) => {
    document
      .querySelectorAll('ul#recipients li')
      .forEach((item) => item.classList.remove('hide'));
    switch (evt.target.value) {
      case 'failure':
        document
          .querySelectorAll('ul#recipients li.success')
          .forEach((item) => item.classList.add('hide'));
        break;
      case 'success':
        document
          .querySelectorAll('ul#recipients li.failed')
          .forEach((item) => item.classList.add('hide'));
        break;
      default:
  ...
  while (recipients.length > 0 && sent.length < recipients.length) {
    ...
    fetch('send-messages', {
      ...
      .then((body) => {
        ...
        console.log(
          `Request ${body.requestId} -- ${successCount} of ${body.result.length}`
        );
        ...
        body.result.map((item) => {
          ...
          if (elem) {
            ...
            console.error(
              `${body.requestId} Could not find element for ${to} Error Message: ${item.error}`
            );
          }
          return item;
        });
