...
function getOptions(params, env, config) {
  ...
  for (const name of Object.keys(env)) {
    ...
    const matches = /^FUNLET_MENU_OPTION([0-9]+)_URL$/.exec(name);
    if (matches !== null) {
...
function simpleMenuStage1(response, message, language, voice, options) {
  let maxDigits = 1;
  for (const digits of Object.keys(options)) {
...
function simpleMenuStage2(
  ...
  const optionUrl = options[digits];
...
exports.handler = function (context, event, callback) {
  ...
  const errorMessage = getErrorMessage(event, context, config);
  const language = getLanguage(event, context, config);
  const voice = getVoice(event, context, config);
  const options = getOptions(event, context, config);
