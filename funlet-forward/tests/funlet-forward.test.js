...
test('[FORWARD-INPUT-ALLOWED-CALLERS-2] '+
...
test('[FORWARD-INPUT-ACCESS-RESTRICTED-1] '+
...
test('[FORWARD-INPUT-ACCESS-RESTRICTED-2] '+
...
test('[FORWARD-INPUT-PHONE-NUMBER-CALLED-1] '+
...
test('[FORWARD-INPUT-PHONE-NUMBER-CALLED-2] '+
...
() => {
  ...
  funlet.output.forwardStage1(
    response,
    IS_ALLOWED_CALLER, DEFAULT_ACCESS_RESTRICTED, ENGLISH, ALICE,
    DEFAULT_CALLER_ID, PHONE_NUMBER, DEFAULT_TIMEOUT, FALLBACK_URL
  );
...
() => {
  let response = new Twilio.twiml.VoiceResponse();
  let isFinished = funlet.output.forwardStage2(
    response, DIAL_DONE, CALL_BUSY, FALLBACK_URL
  );
  expect( isFinished ).toEqual( DIAL_DONE );
