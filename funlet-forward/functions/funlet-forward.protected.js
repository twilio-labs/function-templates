...
function getTimeout(params, env, config) {
  const timeout = params.Timeout || env.FUNLET_FORWARD_TIMEOUT;
...
exports.handler = function (context, event, callback) {
  ...
  const caller = getCaller(event, context, config);
  const called = getPhoneNumberCalled(event, context, config);
  const allowedCallers = getAllowedCallers(event, context, config);
  const accessRestrictedErrorMessage = getAccessRestrictedErrorMessage(
    event,
    context,
    config
  );
  ...
  const callerId = getCallerId(event, context, config);
